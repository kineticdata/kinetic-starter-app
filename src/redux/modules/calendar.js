...
export const types = {
  ...
  REFETCH_CALENDAR_EVENTS: ns('REFETCH_CALENDAR_EVENTS'),
  ...
  SET_RELATED_DATA: ns('SET_RELATED_DATA'),
  SET_FILTERS: ns('SET_FILTERS'),
};
...
export const actions = {
  ...
  refetchCalendarEvents: withPayload(types.REFETCH_CALENDAR_EVENTS),
  ...
  setRelaterData: withPayload(types.SET_RELATED_DATA),
  setFilters: withPayload(types.SET_FILTERS),
};
...
export const reducer = (state, { type, payload }) => {
  ...
  switch (type) {
    ...
    case types.REFETCH_CALENDAR_EVENTS:
      return state.setIn([payload.key, 'loadingEvents'], true);
    ...
    case types.UPDATE_FROM_FILTER:
      state = payload.filters
        ? state.setIn([payload.key, 'filters'], payload.filters)
        : state;
      return state
        .setIn([payload.key, 'events'], payload.events)
    ...
    case types.SET_FILTER_ACTIONS:
      return state
        .setIn([payload.key, 'filterActions'], payload.filterActions)
        .setIn([payload.key, 'filters'], payload.filters);
    case types.SET_RELATED_DATA:
      return state.setIn([payload.key, 'relatedData'], payload.relatedData);
    default:
      return state;
  }
};

export const refetchCalendarEvents = (key, timezone) => {
  store.dispatch(actions.refetchCalendarEvents({ key, timezone }));
};
